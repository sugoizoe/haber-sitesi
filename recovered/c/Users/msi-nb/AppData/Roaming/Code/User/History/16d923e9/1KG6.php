...
<html lang="tr">
      ...
      <section class="hero">
        <article class="lead">
          ...
          <div class="meta">
           ...
           <h1><?= htmlspecialchars($hero['baslik'] ?? 'Henüz haber yok') ?></h1>

            <p>
              <time><?= htmlspecialchars($hero['tarih'] ?? '') ?></time>

            <?= htmlspecialchars($hero['ozet'] ?? '') ?>
          </p>
  ...
  <a href="haber.php?id=<?= (int)$h['id'] ?>" class="mini">
    <div class="title"><?= htmlspecialchars($h['baslik']) ?></div>
    <small><?= htmlspecialchars($h['tarih']) ?></small>
  </a>
<?php endforeach; ?>
